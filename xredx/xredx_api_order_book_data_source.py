...
from .xredx_active_order_tracker import XredxActiveOrderTracker
from .xredx_constants import Constants
from .xredx_order_book import XredxOrderBook
from .xredx_utils import XredxAPIError, convert_from_exchange_trading_pair, convert_to_exchange_trading_pair
from .xredx_websocket import XredxWebsocket


class XredxAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_new_order_book(self, trading_pair: str) -> OrderBook:
        ...
        snapshot_msg: OrderBookMessage = XredxOrderBook.snapshot_message_from_exchange(
            snapshot,
            snapshot_timestamp,
            metadata={"trading_pair": trading_pair})
        order_book = self.order_book_create_function()
        active_order_tracker: XredxActiveOrderTracker = XredxActiveOrderTracker()
    ...
    async def listen_for_trades(self, ev_loop: asyncio.BaseEventLoop, output: asyncio.Queue):
        ...
        while True:
            try:
                ws = XredxWebsocket(throttler=self._throttler)
                    ...
                    if response is not None:
                        for msg_key in list(response.keys()):
                            ...
                            if len(split_key) != 2:
                                ...
                                self.logger().info(f"Unrecognized message received from Xredx websocket: {response}")
                                continue
                            ...
                            for trade in response[msg_key]["trades"]:
                                ...
                                trade_msg: OrderBookMessage = XredxOrderBook.trade_message_from_exchange(
                                    trade,
                                    trade_timestamp,
                                    metadata={"trading_pair": trading_pair})
                    ...
                    if response is not None:
                        for msg_key in list(response.keys()):
                            ...
                            if Constants.WS_METHODS['ORDERS_UPDATE'] in msg_key:
                                order_book_msg_cls = XredxOrderBook.diff_message_from_exchange
                                split_key = msg_key.split(Constants.WS_METHODS['ORDERS_UPDATE'], 1)
                            elif Constants.WS_METHODS['ORDERS_SNAPSHOT'] in msg_key:
                                order_book_msg_cls = XredxOrderBook.snapshot_message_from_exchange
