...
import hummingbot.connector.exchange.xredx.xredx_http_utils as http_utils
from hummingbot.connector.exchange.xredx.xredx_api_order_book_data_source import (
    XredxAPIOrderBookDataSource,
)
from hummingbot.connector.exchange.xredx.xredx_auth import XredxAuth
from hummingbot.connector.exchange.xredx.xredx_constants import Constants
from hummingbot.connector.exchange.xredx.xredx_in_flight_order import XredxInFlightOrder
from hummingbot.connector.exchange.xredx.xredx_order_book_tracker import XredxOrderBookTracker
from hummingbot.connector.exchange.xredx.xredx_user_stream_tracker import XredxUserStreamTracker
from hummingbot.connector.exchange.xredx.xredx_utils import (
    XredxAPIError,
...
class XredxExchange(ExchangeBase):
    """
    XredxExchange connects with Xredx.io exchange and provides order book pricing, user account tracking and
    ...
    def __init__(self,
                 ...
                 xredx_api_key: str,
                 xredx_secret_key: str,
                 trading_pairs: Optional[List[str]] = None,
                 trading_required: bool = True
                 ):
        """
        :param xredx_api_key: The API key to connect to private Xredx.io APIs.
        :param xredx_secret_key: The API secret.
        ...
        self._xredx_auth = XredxAuth(xredx_api_key, xredx_secret_key)
        self._set_order_book_tracker(XredxOrderBookTracker(
            throttler=self._throttler,
            trading_pairs=trading_pairs))
        self._user_stream_tracker = XredxUserStreamTracker(
            throttler=self._throttler,
            xredx_auth=self._xredx_auth,
            trading_pairs=trading_pairs)
        self._ev_loop = asyncio.get_event_loop()
        self._shared_client = None
        self._poll_notifier = asyncio.Event()
        self._last_timestamp = 0
        self._in_flight_orders = {}  # Dict[client_order_id:str, XredxInFlightOrder]
    ...
    @property
    def in_flight_orders(self) -> Dict[str, XredxInFlightOrder]:
    ...
    def restore_tracking_states(self, saved_states: Dict[str, any]):
        ...
        self._in_flight_orders.update({
            key: XredxInFlightOrder.from_json(value)
    ...
    async def _api_request(self,
        ...
        parsed_response = await http_utils.api_call_with_retries(
            method=method,
            endpoint=endpoint,
            auth_headers=self._xredx_auth.get_headers if is_auth_required else None,
            params=params,
        ...
        if "errors" in parsed_response or "error" in parsed_response:
            ...
            raise XredxAPIError(parsed_response)
    ...
    def start_tracking_order(self,
        ...
        """
        self._in_flight_orders[order_id] = XredxInFlightOrder(
    ...
    async def _execute_cancel(self, trading_pair: str, order_id: str) -> CancellationResult:
        ...
        :param trading_pair: The market trading pair (Unused during cancel on Xredx.io)
    ...
    def stop_tracking_order_exceed_not_found_limit(self, tracked_order: XredxInFlightOrder):
        """
    ...
    async def _update_order_status(self):
        ...
        if current_tick > last_tick and len(self._in_flight_orders) > 0:
            ...
            for response, tracked_order in zip(responses, tracked_orders):
                if isinstance(response, XredxAPIError):
    ...
    async def _trigger_order_fill(self,
                                  tracked_order: XredxInFlightOrder,
    ...
    async def all_trading_pairs(self) -> List[str]:
        ...
        return await XredxAPIOrderBookDataSource.fetch_trading_pairs()

    async def get_last_traded_prices(self, trading_pairs: List[str]) -> Dict[str, float]:
        # This method should be removed and instead we should implement _get_last_traded_price
        return await XredxAPIOrderBookDataSource.get_last_traded_prices(
            trading_pairs=trading_pairs,
            throttler=self._throttler
        )
